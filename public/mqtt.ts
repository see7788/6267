...
import { ObjBase_t, ObjExtendsReturninfer_t, send_t, call_t, resFunCreate } from "./ts"
type MqttCreateParam_t = { url: string, op: mqttClient.IClientOptions, connect: (c: mqttClient.IConnackPacket) => void }
export default class {
  private obj
  private clientId
  disconnect
  constructor({ url, op, connect }: MqttCreateParam_t) {
  ...
  subscribe<T extends ObjBase_t>(api?: T) {
    return this.obj.subscribe(
      `${String(api)}/${this.clientId}`,  //通配符#+在//之间或者末尾:#任意层，+一层
      {
  ...
  publishInit<T extends ObjBase_t>(): send_t<T> {
